      ...
      <div className="hidden lg:block p-6">
        ...
        <div className="grid gap-5" style={{ gridTemplateColumns: '260px 1fr 300px' }}>
          ...
          <aside className="space-y-4">
            ...
            <div className="ny-card border ny-border rounded-xl p-4">
              <div className="ny-text-3 text-xs font-semibold tracking-wide uppercase mb-3">Templates</div>
              <div className="space-y-1.5">
                {TEMPLATES_LIST.map(([id, label]) => (
                  <button key={id} onClick={() => setSelectedTemplate(id)}
                    className={`w-full text-left px-3 py-2 rounded-lg border text-sm transition-all ${selectedTemplate === id ? 'border-purple-400/70 bg-purple-500/10 ny-text-1' : 'ny-border ny-text-2 hover:bg-white/5'}`}>
                    {label}
                  </button>
                ))}
              </div>
            ...
            <div className="ny-card border ny-border rounded-xl p-4">
              <div className="ny-text-3 text-xs font-semibold tracking-wide uppercase mb-3">Export</div>
              <div className="flex flex-col gap-2">
                <button onClick={exportPdf} className="w-full ny-btn-primary rounded-lg py-2 text-sm flex items-center justify-center gap-2"><Download className="w-4 h-4" />Save as PDF</button>
                <button onClick={exportDoc} className="w-full ny-btn-secondary rounded-lg py-2 text-sm flex items-center justify-center gap-2"><FileText className="w-4 h-4" />Save as DOCX</button>
              </div>
              <p className="ny-text-3 mt-2" style={{ fontSize: 11 }}>PDF keeps the template look. DOCX is a clean, editable Word version.</p>
            </div>
          </aside>
        ...
        <style>{`
          @media print {
            ...
            #ws-resume, #ws-resume * { visibility: visible !important; }
            .ws-pv-wrap { height: auto !important; overflow: visible !important; }
            .ws-pv-page { transform: none !important; }
            #ws-resume { position: absolute !important; left: 0 !important; top: 0 !important; width: 794px !important; box-shadow: none !important; border-radius: 0 !important; }
            @page { margin: 0.5in; }
          }
        `}</style>
      </div>
